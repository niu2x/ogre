...
void ApplicationContextAndroid::_fireInputEventAndroid(AInputEvent* event, int wheel) {
    ...
    if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_MOTION) {
        ...
        evt.tfinger.x = AMotionEvent_getRawX(event, 0) / win->width() * win->getViewPointToPixelScale();
        evt.tfinger.y = AMotionEvent_getRawY(event, 0) / win->height() * win->getViewPointToPixelScale();
