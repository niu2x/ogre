...
Ogre::Real Widget::getCaptionWidth(const Ogre::DisplayString &caption, Ogre::TextAreaOverlayElement *area)
{
    ...
    for (unsigned int i = 0; i < caption.length(); i++)
    {
        ...
        if (caption[i] == ' ')
        {
            ...
            else lineWidth += font->get_glyph_info(' ').advance * area->getCharHeight();
        }
        else if (caption[i] == '\n') break;
        // use glyph information to calculate line width
        else lineWidth += font->get_glyph_info(caption[i]).advance * area->getCharHeight();
    }
...
void Widget::fitCaptionToArea(const Ogre::DisplayString &caption, Ogre::TextAreaOverlayElement *area, Ogre::Real maxWidth)
{
    ...
    for (unsigned int i = 0; i < s.length(); i++)
    {
        ...
        else width += f->get_glyph_info(s[i]).advance * area->getCharHeight();
        if (width > maxWidth)
        {
            s = s.substr(0, i);
            break;
...
void TextBox::setText(const Ogre::DisplayString &text)
{
    ...
    for (unsigned int i = 0; i < current.length(); i++)
    {
        if (current[i] == ' ')
        {
            ...
            else lineWidth += font->get_glyph_info(' ').advance * mTextArea->getCharHeight();
            firstWord = false;
            lastSpace = i;
        }
        else if (current[i] == '\n')
        ...
        {
            // use glyph information to calculate line width
            lineWidth += font->get_glyph_info(current[i]).advance * mTextArea->getCharHeight();
