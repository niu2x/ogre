...
void DotSceneLoader::load(DataStreamPtr& stream, const String& groupName, SceneNode* rootNode)
{
    ...
    if (!result)
    {
        LogManager::singleton().log_error(
            "DotSceneLoader - " + String(result.description()));
        return;
    }
...
void DotSceneLoader::processScene(pugi::xml_node& XMLRoot)
{
    ...
    LogManager::singleton().log_message(message);
...
#ifdef OGRE_BUILD_COMPONENT_TERRAIN
    LogManager::singleton().log_message(
        "[DotSceneLoader] Processing Terrain Group...",
        LogMsgLevel::TRIVIAL);
...
void DotSceneLoader::processLookTarget(pugi::xml_node& XMLNode, SceneNode* pParent)
{
    ...
    LogManager::singleton().log_message(
        "[DotSceneLoader] Processing Look Target, nodeName: " + nodeName,
        LogMsgLevel::TRIVIAL);
...
void DotSceneLoader::processTrackTarget(pugi::xml_node& XMLNode, SceneNode* pParent)
{
    ...
    LogManager::singleton().log_message(
        "[DotSceneLoader] Processing Track Target, nodeName: " + nodeName,
        LogMsgLevel::TRIVIAL);
		...
		if (!instancedManager.empty())
		{
            LogManager::singleton().log_message(
                "[DotSceneLoader] Adding entity: " + name
                    + " to Instance Manager: " + instancedManager,
                LogMsgLevel::TRIVIAL);
            ...
            if (!staticGeometry.empty()) {
                ...
                mSceneMgr->getStaticGeometry(staticGeometry)
                    ->addEntity(
                        static_cast<Entity*>(pEntity),
                        pParent->_getDerivedPosition(),
                        pParent->_getDerivedOrientation(),
                        pParent->_getDerivedScale());
            } else {
                LogManager::singleton().log_message(
                    "[DotSceneLoader] pParent->attachObject(): " + name,
                    LogMsgLevel::TRIVIAL);
...
{
    LogManager::singleton().log_message(
        "[DotSceneLoader] Processing SkyPlane...",
        LogMsgLevel::TRIVIAL);
...
{
    LogManager::singleton().log_message(
        "[DotSceneLoader] Processing Node Animations for SceneNode: "
            + pParent->name(),
        LogMsgLevel::TRIVIAL);
...
void DotSceneLoader::processNodeAnimation(pugi::xml_node& XMLNode, SceneNode* pParent)
{
    ...
    LogManager::singleton().log_message(
        "[DotSceneLoader] Processing Node Animation: " + name,
        LogMsgLevel::TRIVIAL);
    ...
    if (interpolationMode == "linear")
        ...
        LogManager::singleton().log_error(
            "DotSceneLoader - Invalid interpolationMode: " + interpolationMode);
    ...
    if (rotationInterpolationMode == "linear")
        ...
        LogManager::singleton().log_error(
            "DotSceneLoader - Invalid rotationInterpolationMode: "
...
void DotSceneLoader::processKeyframe(pugi::xml_node& XMLNode, NodeAnimationTrack* pTrack)
{
    ...
    LogManager::singleton().log_message(
        "[DotSceneLoader] Processing Keyframe: "
            + StringConverter::to_string(time),
        LogMsgLevel::TRIVIAL);
...
void DotSceneLoader::writeNode(pugi::xml_node& parentXML, const SceneNode* n)
{
    ...
    for(auto mo : n->getAttachedObjects())
    {
        ...
        LogManager::singleton().log_warning(
            "DotSceneLoader - unsupported MovableType " + mo->getMovableType());
    }
...
void DotScenePlugin::shutdown() {
    ...
    extern "C" _OgreDotScenePluginExport void dllStartPlugin()
    {
        Ogre::Root::singleton().installPlugin(&plugin);
    }
    extern "C" _OgreDotScenePluginExport void dllStopPlugin()
    {
        Ogre::Root::singleton().uninstallPlugin(&plugin);
    }
#endif
